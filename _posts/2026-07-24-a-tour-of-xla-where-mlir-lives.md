...
It is a companion to [the tour of MLIR]({% post_url 2026-06-23-mlir-dialect-stack-for-ml %}): that post was about MLIR as a general IR construction kit; this one is about one large, mature consumer adopting it piece by piece. Everything below is grounded in the `openxla/xla` source tree, with paths cited.
...
The `^bb0` block, the region in `({ ... })`, the `#stablehlo<...>` attribute, the trailing type signature — none of that is XLA's invention. StableHLO did not merely *use* MLIR as a library; it adopted MLIR's textual and structural conventions as the interchange format the whole ecosystem now targets. That is about as complete an endorsement of an IR's design as one project can give another. And it is the front door XLA translates through to reach its classic-HLO core.
...
And the door has become a doorway others walk through. PyTorch, working with Google, shipped a TPU backend for **Helion** — its high-level kernel-authoring DSL — that "compiles Helion kernels to Pallas."[^13] So a PyTorch author writing a single Helion kernel reaches XLA's Mosaic TPU backend *through* JAX's Pallas — `Helion → Pallas → Mosaic → TPU` — with the explicit aim of keeping "the same set of kernels across TPU and GPU." The escape hatch is now a convergence point: two frameworks and two kernel DSLs stacked onto one shared set of MLIR codegen backends. The same pull that drew a mature XLA to MLIR draws the kernel languages onto XLA's MLIR backends in turn.
...
MLIR gives XLA a great deal: a portable, versioned front door whose syntax the whole ecosystem now shares, a real dialect for the hard problem of sharding, a reusable progressive-lowering path to LLVM, and a set of codegen backends (Triton, Mosaic) that even a hand-written Pallas kernel can reach. It is why XLA can retarget and why its codegen is increasingly shared rather than hand-written. But it is worth ending where the MLIR tour did, because XLA is the proof of it. MLIR is plumbing: it gives XLA the machinery to *represent* StableHLO, `sdy` shardings, and TPU memory spaces, and to lower them. It does not decide that a tensor should live on a particular device, or in VMEM rather than HBM, and it does not put any of that in the *type* of the value. Those remain, in XLA as everywhere, annotations and dialect attributes checked by passes — the `sdy` sharding on an op, the memory space on a Mosaic `Ref` — rather than properties the type system enforces where you wrote the program. That gap is the one I keep coming back to.

The map, in one line: StableHLO in, Shardy to the side, emitters and Triton and Mosaic (and cuBLAS/cuDNN) out, Pallas as a side-door to the same backends — and a mature C++ HLO optimizer still in the middle, quietly where most of the compiling happens. The through-line is the more telling part. XLA did not need MLIR; it had a complete compiler of its own. It adopted MLIR anyway, at each layer where the ecosystem offered something its own IR did not — a portable versioned interchange, a real dialect for sharding, a reusable lowering path, a shared set of kernel backends. A mature compiler voting with its own architecture is about the most concrete measure of MLIR's pull there is.
