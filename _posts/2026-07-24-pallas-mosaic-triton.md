...
On a machine with a working, CUDA-enabled `jax` that reports `[CudaDevice(id=0)]`. The Triton backend resolves hardware by matching the device's name string against a regex assembled from a hardcoded enum of nineteen product names, and if that fails it raises.[^4] So:
...
[^4]: **The Pallas Triton device allowlist.** The Triton backend resolves hardware by matching `device_kind` against a regex built from the `GpuVersion` enum — [`gpu_info.py#L55`](https://github.com/jax-ml/jax/blob/jax-v0.11.0/jax/_src/pallas/triton/gpu_info.py#L55) is `re.compile(r"\b(" + "|".join(e.value for e in GpuVersion) + r")\b")`, so the pattern is generated from a hardcoded list of nineteen product names and anchored with `\b` at both ends. The fallback `registry` dict is empty. A non-matching name raises from [`pallas_call_registration.py#L107`](https://github.com/jax-ml/jax/blob/jax-v0.11.0/jax/_src/pallas/triton/pallas_call_registration.py#L107); in that same function the `try: gpu_info = gpu_info_lib.get_gpu_info()` block precedes the `lower_jaxpr_to_triton_module(...)` call, so the lookup failure short-circuits before any Triton lowering is attempted. Links pinned to the `jax-v0.11.0` tag, which is the release every line number here refers to.
