...
`tpu_ops.td` defines **90 operations**, and they are not all vector work. Roughly half — 48 — are vector-unit ops; the rest are the MXU FIFO, DMA and semaphores, memref manipulation, identity queries, and debug, each of which gets its own section below.[^8] That vector subset is the closest thing to a public TPU vector ISA, and reading it tells you which primitives the hardware does natively. Grouped:
