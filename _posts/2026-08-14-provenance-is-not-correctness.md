...
Sample twice from an unchanged distribution and you get two different programs. Both are legitimate draws. Neither is more likely to be correct than the other, and the model's distribution over programs already contains buggy programs. A watermark that samples faithfully from that distribution has not made your code worse. It has given you a different draw from a bag that always contained bad draws.

So the naive claim — watermarking injects vulnerabilities into generated code — is simply wrong, and it should be discarded before anything is built on top of it. A distribution-preserving watermark does not bias generation toward insecure constructs. There is no adversary in the sampler.

The problem is structural, and you find it by asking where a watermark can hide.
...
Now suppose a systematic defect is later found in some model's output, a window during which it reliably emitted a weak cipher default, a broken sanitizer, an off-by-one in a common idiom. Models do have characteristic failure modes, and those modes change between versions. At that moment the party best positioned to enumerate the blast radius is the party that caused it, and the parties who need to remediate are the ones who cannot. A key that identifies "code generated by model M during window W" becomes a vulnerability inventory once W has a known defect. Whoever holds it holds that inventory, which makes the key a concentrated target for breach, insider abuse, subpoena, or state compulsion, and makes its custody a security property of every downstream codebase.
