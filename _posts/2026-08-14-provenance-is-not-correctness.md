...
That is worth reading as a systems fact rather than a policy announcement. A watermark is not a label attached to a document. It is an intervention in the sampling loop. Something is changing which token gets emitted, and that is a different kind of object from a metadata field.

The interesting question is what happens when the artifact being marked is a program.

## What a token watermark actually does

Anthropic has not published its scheme. The documentation says details on detection mechanisms are coming in "forthcoming technical documentation,"[^1] so what follows describes the published literature on token-level watermarking rather than any specific implementation. That literature is small and consistent enough to reason from.
...
A watermark is an active intervention. It arbitrages which code patterns get selected. You might have two equally valid ways to structure a loop or initialize a struct. Both are legitimate draws from the unwatermarked distribution. The watermark tilts the sampler toward the one that hashes into the green list.

This affects the actual code generated. And code patterns age poorly. An idiom that looks safe today becomes tomorrow's vulnerability when a new compiler optimization or attack surface is discovered.

Because the watermark steered the generation toward specific patterns to construct its signal, the originator knows exactly which choices were made. If one of those patterns is later found to be vulnerable, the originator knows about all the places that have those vulnerabilities. That is risky.

The problem is structural. You find the bounds of that risk by asking where a watermark can hide.
...
Code is not like that. Enormous stretches are nearly deterministic given context, and a large share of the positions where the model *is* confident are the positions that carry the semantics:
...
**Detection is an oracle, and oracles cut both ways.** Once third-party detection tooling exists, anyone who can run it against a repository can label which regions are model-generated. That is a targeting signal. An attacker who knows which subsystems were machine-written knows where to look first, because model-generated code has characteristic weaknesses. Provenance transparency is not free; it tells your adversary something too.

**The key holder can enumerate; nobody else can.** Detection requires the watermark key, and the originator holds it. That asymmetry exists today, before any third-party tooling ships.

A provider already knows what it generated, because request logs give you that. What a watermark adds is recognition of your own output *after it has left your infrastructure*: committed to a repository, published in a package, vendored into someone else's tree, pasted into a Stack Overflow answer. Logs cover your account boundary. A watermark covers anywhere you can see.

Now suppose a systematic defect is later found in one of those watermarked idioms. A weak cipher default, a broken sanitizer, an off-by-one error. The watermark didn't just passively observe the generation; it arbitrated the choice that selected the vulnerable construct.

At that moment the party best positioned to enumerate the blast radius is the party holding the key, and the parties who need to remediate are the ones who cannot. A key that maps exactly where that pattern was injected becomes a vulnerability inventory. Whoever holds it holds that inventory, which makes the key a concentrated target for breach, insider abuse, subpoena, or state compulsion, and makes its custody a security property of every downstream codebase.

That is a structural property of keyed watermarking, not an allegation that anyone scans repositories this way. The entropy argument bounds it, too: if code watermarks weakly, enumeration over code is correspondingly weak, and the same physics that makes the signal a poor assurance tool makes it a poor census. But the direction of the asymmetry is fixed, and it runs against the people holding the liability.
...
One further possibility belongs in the speculation column. Because the green/red partition is derived deterministically from preceding tokens, an adversary who knew both the scheme and the key might in principle craft a prefix such that at a chosen decision point the vulnerable token is favoured and the safe one disfavoured. I have not seen a demonstration against a deployed system, the δ in a soft watermark is small relative to the model's confidence at the low-entropy positions that would matter, and Anthropic's key is not public. Treat it as a research question, not a finding.

## Thompson settled this in 1984

Suppose every objection above were answered. Suppose detection is flawless, scrubbing impossible, spoofing infeasible, and every line of machine-written code in your repository is reliably attributable.
...
No trace in source anywhere. The source of the compiler is clean. The source of `login` is clean. Every line a reviewer can read is clean, and the system is owned. Then the moral:
...
Now put a watermark next to that sentence. A watermark is a statistical property of a token sequence in the source. It is strictly *weaker* than source-level scrutiny, since reading the source at least tells you what the source says. Thompson demolished the entire category of source-level assurance in 1984, and a watermark is a new instrument aimed at the layer he had already shown to be the wrong one.

He also anticipated the generalization, in a line usually dropped from the retelling:
...
*Any program-handling program.* A code-generating model is now the first program-handling program in the chain, earlier than the compiler, and operating on intent rather than source. That is a new link in the supply chain Thompson was describing, and the industry's response so far is to mark its output and call the problem addressed.

## Check the artifact, not the author

Thompson posed the problem; Necula proposed the shape of an answer thirteen years later. Proof-carrying code, from POPL 1997, addresses running a program from an untrusted source: the producer ships the code together with a machine-checkable proof that it satisfies the consumer's safety policy, and the consumer verifies the proof before running it.[^5] The producer's trustworthiness never enters the argument, which is the only way out of Thompson's box, since every alternative reduces to trusting somebody. Checking is cheap, automatic, and independent of who, or what, did the generating.

The one concrete countermeasure to Thompson's specific attack has the same shape. David A. Wheeler's diverse double-compiling recompiles the source twice, once with a second and independently produced compiler, then again using the result, so a subverted binary is detectable by comparison rather than by trust.[^9] The ACSAC 2005 paper gave an informal justification; his 2009 dissertation supplied a formal proof that the technique works. The answer to "can I trust this artifact" turned out to need a proof, not a provenance record.

That is the shape of the problem an LLM creates, and it is why the answer is not better provenance.

There is also a useful asymmetry in how models and verifiers fail. A language model is very good at producing a plausible candidate and structurally incapable of guaranteeing anything about it. A verifier is hopeless at producing candidates and extremely good at rejecting wrong ones. Generate-and-check pairs the two along their strengths, and it improves as generation gets cheaper, because you can afford to throw away rejected candidates. The bottleneck moves to the specification, which is where you wanted it.

## What verification buys, and what it costs

The strongest empirical case for verification and the sharpest limit on it come from the same paper.
...
CompCert, the formally verified C compiler, is the outlier in that study, and the authors are careful about exactly how:
...
The bugs were in the parts nobody had proved. Their conclusion is the sentence I would put above this whole discussion:
...
Carry that expectation into an LLM-generated codebase. A proof covers what it covers. The trusted computing base is where your remaining bugs live, and machine-generated code expands that surface faster than anyone is expanding proofs.

The cost is real too. seL4, the first machine-checked functional-correctness proof of a general-purpose OS kernel, covers roughly 8,700 lines of C and 600 lines of assembler.[^7] That is a small kernel and a very large effort, and nobody is verifying a web service that way. Specifications are also code: they can be wrong, they can be vacuous, and a proof against a wrong spec buys you nothing. Anyone selling formal methods as a silver bullet is selling something.
...
Three things, and none of them requires a verification team.
...
**Ask for self-service detection, and ask early.** The key-holder asymmetry is the one item here you cannot engineer around from inside your own repository, because it is a property of who holds the key. The mitigation is contractual, and it should be raised while detection tooling is still being designed: if a provider can recognise its output in your codebase, you should be able to recognise it too, on your own infrastructure, without shipping your source anywhere. Any scheme that only works as a service run by the key holder leaves you unable to audit your own exposure at the moment a systematic defect is disclosed.
...
The watermark tells you where a program came from. It was never going to tell you what the program does, and on code it turns out to be poor at even the first job. The durable answer is the one Necula gave in 1997: do not trust the producer, check the artifact.
...
[^8]: **Thompson, "Reflections on Trusting Trust."** Turing Award Lecture, *Communications of the ACM* 27, no. 8 (August 1984), pp. 761–763. Source of the opening question, the `login` backdoor construction, the self-reproducing compiler Trojan that leaves "no trace in source anywhere," the moral that "no amount of source-level verification or scrutiny will protect you from using untrusted code," and the generalization to "any program-handling program such as an assembler, a loader, or even hardware microcode." ([ACM DL](https://dl.acm.org/doi/10.1145/358198.358210), [PDF](https://www.cs.cmu.edu/~rdriley/487/papers/Thompson_1984_ReflectionsonTrustingTrust.pdf))
